...
class PerformanceEnv:
    def __init__(self, delay_mean, delay_std):
        ...
        self.observation_space = gymnasium.spaces.Box(low=-(2**20), high=2**20, shape=(1,), dtype=np.float32)
...
def test_performance(vectorization, workers, envs_per_worker, delay_mean, delay_std, batch_size=None, timeout=1):
    def make_env():
        return pufferlib.emulation.GymnasiumPufferEnv(env_creator=PerformanceEnv, env_args=(delay_mean, delay_std))
            ...
            for vec in backends:
                ...
                for envs_per_worker in (1, 2, 4):
                    ...
                    for batch in batch_sizes:
                        steps, duration = test_performance(vec, workers, envs_per_worker, mean, std, batch)

                        res[(envs_per_worker, batch)] = (steps, duration)

                        print("SPS, envs/worker, batch size: ", steps / duration, envs_per_worker, batch)
...
def plot_performance_tests():
    ...
    layout = go.Layout(
        title=dict(text="Performance of Vectorization Backends on Various Workloads (24 core machine)", y=0.9),
        width=2000,  # 1000,
        height=500,

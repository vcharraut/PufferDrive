...
from pufferlib.vector import Multiprocessing, Serial, make
...
def profile_emulation(env_creator, timeout=DEFAULT_TIMEOUT, seed=42):
    ...
    env_overhead = (
        100 * (puf_total_step - raw_total_step + puf_total_reset - raw_total_reset) / (puf_total_step + puf_total_reset)
    )
...
def profile_sb3_vec(env_creator, num_envs, timeout=DEFAULT_TIMEOUT):
    with pufferlib.utils.Suppress():
        ...
        actions = [[vecenv.action_space.sample() for _ in range(num_envs)] for _ in range(1000)]
...
def profile_all(
    name, env_creator, num_envs, num_workers=24, env_batch_size=None, zero_copy=True, timeout=DEFAULT_TIMEOUT
):
...
if __name__ == "__main__":
    ...
    # profile_emulation(env_creator)
    profile_puffer(env_creator, num_envs=24, batch_size=8, backend=Multiprocessing, zero_copy=False)
    ...
    profile_emulation(env_creator)
    # profile_puffer(env_creator, num_envs=8, backend=Multiprocessing)
    profile_puffer(env_creator, num_envs=96, batch_size=48, backend=Multiprocessing, zero_copy=False)
    print()
    ...
    profile_all("Pokemon Red", pokemon_red.env_creator(), num_envs=144, env_batch_size=48, zero_copy=False)

    from pufferlib.environments import procgen

    profile_all(
        "ProcGen", procgen.env_creator("bigfish"), num_envs=144, env_batch_size=48, num_workers=24, zero_copy=False
    )

    from pufferlib.environments import classic_control

    profile_all("Classic Control", classic_control.env_creator(), num_envs=1152, env_batch_size=48)

    from pufferlib.environments import ocean

    profile_all("Ocean Squared", ocean.env_creator("squared"), num_envs=1152, env_batch_size=48)

    from pufferlib.environments import atari

    profile_all(
        "Atari Breakout", atari.env_creator("BreakoutNoFrameskip-v4"), num_envs=144, env_batch_size=48, zero_copy=False
    )

    from pufferlib.environments import crafter

    profile_all("Crafter", crafter.env_creator(), num_envs=24, env_batch_size=8, zero_copy=False)

    from pufferlib.environments import minigrid

    profile_all("MiniGrid", minigrid.env_creator(), num_envs=192, env_batch_size=48, zero_copy=False)

    ...
    gym.spaces.Dict(
        {
            ...
            "c": gym.spaces.Dict({"d": gym.spaces.Box(low=-10, high=10, shape=(100,)), "e": gym.spaces.Discrete(1000)}),
        }
    ),
    ...
    gym.spaces.Dict(
        {
            ...
            "h": gym.spaces.Tuple(
                (
                    ...
                    gym.spaces.Dict(
                        {"i": gym.spaces.Discrete(5000), "j": gym.spaces.Box(low=-100, high=100, shape=(10000,))}
                    ),
                )
            ),
        }
...
def test_flatten_unflatten(iterations=10_000):
    ...
    for space in test_cases:
        ...
        flatten_times.append(timeit.timeit(lambda: flatten(data), number=iterations))
        concatenate_times.append(timeit.timeit(lambda: concatenate(flat), number=iterations))
        split_times.append(timeit.timeit(lambda: split(merged, flat_space, sz, batched=False), number=iterations))
        unflatten_times.append(timeit.timeit(lambda: unflatten(unmerged, structure), number=iterations))

...
def test_gymnasium_api(print_errors=False):
    env = pufferlib.emulation.GymnasiumPufferEnv(env_creator=test.GymnasiumTestEnv)
...
def test_pettingzoo_api_usage(print_errors=False):
    env = pufferlib.emulation.PettingZooPufferEnv(env_creator=test.PettingZooTestEnv)
...
def test_vectorization_api(print_errors=False):
    gymnasium_creator = lambda: pufferlib.emulation.GymnasiumPufferEnv(env_creator=test.GymnasiumTestEnv)
    pettingzoo_creator = lambda: pufferlib.emulation.PettingZooPufferEnv(env_creator=test.PettingZooTestEnv)

    for backend in [pufferlib.vector.Serial, pufferlib.vector.Multiprocessing, pufferlib.vector.Ray]:
        for creator in [gymnasium_creator, pettingzoo_creator]:
            vec = pufferlib.vector.make(creator, num_envs=6, num_workers=3, backend=backend)
            ...
            # Async API
            vec = pufferlib.vector.make(creator, num_envs=8, num_workers=4, batch_size=4, backend=backend)
            vec.async_reset()
            actions = vec.action_space.sample()
        ...
        try:
            vec = pufferlib.vector.make(gymnasium_creator, num_envs=3, num_workers=2)
        except APIUsageError as e:
            print_if(e, print_errors)

        try:
            vec = pufferlib.vector.make(gymnasium_creator, num_envs=4, num_workers=2, batch_size=3)
        except APIUsageError as e:
            print_if(e, print_errors)
